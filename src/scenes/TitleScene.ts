 ...
 * 1人・2人・タイムアタック・フリープレイの4つと、オプションが並ぶ。
 * 上下で選び、決定は Enter（パッドは ○／A）、戻るは Esc（×／B）。
 *
 * **コンピュータの腕前はここには置かない**（2026-08-15 改定）。
 * オプションの「コンピュータの強さ」に一本化してある
 */
...
interface Mode {
  name: string;
  note: string;
  /** 移る画面。タイムアタックとオプションは別の画面へ行く */
  scene: 'Play' | 'TimeAttack' | 'Options';
  /** 相手を出さず、一人で飛ぶだけにするか */
  free?: boolean;
...
const MODES: Mode[] = [
  { name: '1人で遊ぶ', note: '2P はコンピュータ（強さはオプションで）', scene: 'Play',
    p1Ai: () => 0, p2Ai: (lv) => lv },
  { name: '2人で対戦', note: '1台のキーボード、またはパッド2台', scene: 'Play',
    p1Ai: () => 0, p2Ai: () => 0 },
  { name: 'タイムアタック', note: '輪をくぐって10ステージの合計タイムを競う', scene: 'TimeAttack',
    p1Ai: () => 0, p2Ai: () => 0 },
  { name: 'フリープレイ', note: '相手なし。一人で飛ぶだけ', scene: 'Play', free: true,
    p1Ai: () => 0, p2Ai: () => 0 },
  { name: 'オプション', note: '音・見た目・ルール・機体の性能・パッドの割り当て', scene: 'Options',
    p1Ai: () => 0, p2Ai: () => 0 },
];
